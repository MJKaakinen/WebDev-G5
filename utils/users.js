...
const getUser = (email, password) => {
  const user = data.users.find(wantedUser => wantedUser.email === email && wantedUser.password === password);
  return user && {...user };
};
...
const deleteUserById = userId => {
  ...
  const index = data.users.findIndex( user => { return user._id === userId;});
  if (index < 0) {
    return; 
  }
...
const saveNewUser = user => {
  ...
  //fixed to prevent admin status on registration
  if (!newUser.role || (newUser.role && newUser.role.toUpperCase() !== 'CUSTOMER') ) newUser.role = 'customer';
...
const updateUserRole = (userId, role) => {
  if (data.roles.includes(role)) {
    const wantedUser = data.users.find(user => user._id === userId);
    if (typeof wantedUser === 'undefined') {
      return;
    }
    wantedUser.role = role;
    return {...wantedUser};
  }
  else {
    throw "Unknown role";
