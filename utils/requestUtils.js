...
const getCredentials = request => {
  ...
  const auth = extractAuthorization(request);
  if (auth && auth[0].toUpperCase() === 'BASIC') {
    const credentialsAs64 = auth[1];
    const credentialsAsString = decodeFrom64(credentialsAs64);
    const credentialsArray = credentialsAsString.split(':');
    return credentialsArray;
  }
  return null;
};

//helper method, extracts authorization info from headers into an array
...
//helper method, decodes base64
const decodeFrom64 = encodedAs64 => {
  const buff = Buffer.from(encodedAs64, 'base64');
