...
let dataAsJSON;
try {
  dataAsJSON = JSON.parse(fs.readFileSync('./products.json', 'utf8'));
}
catch (err) {
  console.log(err);
  dataAsJSON = null;
...
const handleRequest = async(request, response) => {
  ...
  if (matchUserId(filePath)) {
    ...
    if(typeof user === "undefined") return responseUtils.notFound(response);
    if (method.toUpperCase() === 'OPTIONS') return sendOptions(filePath, response);
    if (method.toUpperCase() === 'GET'){
      return responseUtils.sendJson(response, user);
    }
    if (method.toUpperCase() === 'PUT'){
      const reqBodyAsJson = await parseBodyJson(request);
      const role = reqBodyAsJson.role;
      try {
  ...
  if (filePath === '/api/products' && method.toUpperCase() === 'GET') {
    ...
    return responseUtils.sendJson(response, dataAsJSON);
